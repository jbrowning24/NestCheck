#!/usr/bin/env python3
"""
Validate HPMS high-traffic road ground-truth against the actual evaluator.

Loads ground-truth JSON from the generator, runs check_high_traffic_road()
for each test coordinate, and compares actual vs. expected results.

No API calls — uses only spatial.db via SpatialDataStore.

Usage:
    python scripts/validate_ground_truth_hpms.py
    python scripts/validate_ground_truth_hpms.py --input data/ground_truth_hpms.json
    python scripts/validate_ground_truth_hpms.py --verbose
    python scripts/validate_ground_truth_hpms.py --output data/validation_results_hpms.json
"""
...
from property_evaluator import check_high_traffic_road, CheckResult
from spatial_data import SpatialDataStore


def main():
    parser = argparse.ArgumentParser(
        description="Validate HPMS ground-truth against evaluator"
    )
    parser.add_argument(
        "--input", type=str, default="data/ground_truth_hpms.json",
        help="Ground-truth JSON file (default: data/ground_truth_hpms.json)",
    )
    parser.add_argument(
    ...
    for entry in addresses:
        ...
        hpms_expected = entry["tier1_health_checks"]["high_traffic_road"]
        expected_result = hpms_expected["expected_result"]

        # Run the actual check
        tier1 = check_high_traffic_road(lat, lng, spatial_store)
        actual_result = tier1.result.value  # "PASS", "FAIL", "WARNING", "UNKNOWN"
        ...
        # Extract AADT and distance from the Tier1Check value dict
        actual_aadt = None
        actual_dist_ft = None
        actual_road = None
        actual_radius_m = None
        if isinstance(tier1.value, dict):
            actual_aadt = tier1.value.get("aadt")
            actual_dist_ft = tier1.value.get("distance_ft")
            actual_road = tier1.value.get("road_name")
            actual_radius_m = tier1.value.get("radius_m")
        ...
        result_entry = {
            ...
            "actual_aadt": actual_aadt,
            "actual_distance_ft": actual_dist_ft,
            "actual_radius_m": actual_radius_m,
            "actual_road_name": actual_road,
        ...
        if args.verbose:
            ...
            aadt_str = f"AADT {actual_aadt:,}" if actual_aadt is not None else "AADT N/A"
            dist_str = f"{actual_radius_m:.0f}m" if actual_radius_m is not None else "N/A"
            print(
                f"[{marker}] {test_id}: expected={expected_result} "
                f"actual={actual_result} dist={dist_str} {aadt_str}"
            )
            if status == "MISMATCH":
                src = entry.get("source_segment", {})
                print(
                    f"       Generated {src.get('distance_m', '?')}m from "
                    f"'{src.get('name', '?')}' (AADT {src.get('aadt', '?'):,}) | "
                    f"actual nearest: {dist_str} {aadt_str} "
                    f"road={actual_road or 'N/A'}"
                )
    ...
    if mismatch_entries:
        ...
        for m in mismatch_entries[:20]:  # Cap at 20
            src = m.get("source_segment", {})
            aadt_str = (
                f"AADT {m['actual_aadt']:,}" if m["actual_aadt"] is not None
                else "AADT N/A"
            )
            dist_str = (
                f"{m['actual_radius_m']:.0f}m" if m["actual_radius_m"] is not None
                else "N/A"
            )
            print(
                f"  {m['id']}: expected={m['expected_result']} "
                f"actual={m['actual_result']} "
                f"actual_dist={dist_str} {aadt_str} "
                f"(generated {src.get('distance_m', '?')}m from "
                f"'{src.get('name', '?')}' AADT {src.get('aadt', '?'):,})"
            )
