#!/usr/bin/env python3
"""
Validate grocery/provisioning scoring ground-truth against actual functions.

Loads ground-truth JSON from the generator, runs apply_piecewise() for each
test case, and compares actual vs expected results.

No API calls, no spatial.db — pure scoring function tests.

Usage:
    python scripts/validate_ground_truth_grocery.py
    python scripts/validate_ground_truth_grocery.py --input data/ground_truth/grocery.json
    python scripts/validate_ground_truth_grocery.py --verbose
    python scripts/validate_ground_truth_grocery.py --output data/validation_results_grocery.json
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate grocery ground-truth against scoring functions"
    )
    parser.add_argument(
        "--input", type=str, default="data/ground_truth/grocery.json",
        help="Ground-truth JSON file (default: data/ground_truth/grocery.json)",
    )
    parser.add_argument(
