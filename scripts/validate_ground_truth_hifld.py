#!/usr/bin/env python3
"""
Validate HIFLD power line ground-truth against the actual evaluator.

Loads ground-truth JSON from the generator, runs check_hifld_power_lines()
for each test coordinate, and compares actual vs. expected results.

No API calls — uses only spatial.db via SpatialDataStore.

Usage:
    python scripts/validate_ground_truth_hifld.py
    python scripts/validate_ground_truth_hifld.py --input data/ground_truth/hifld.json
    python scripts/validate_ground_truth_hifld.py --verbose
    python scripts/validate_ground_truth_hifld.py --output data/validation_results_hifld.json
"""
...
from property_evaluator import check_hifld_power_lines
from spatial_data import SpatialDataStore


def main():
    parser = argparse.ArgumentParser(
        description="Validate HIFLD ground-truth against evaluator"
    )
    parser.add_argument(
        "--input", type=str, default="data/ground_truth/hifld.json",
        help="Ground-truth JSON file (default: data/ground_truth/hifld.json)",
    )
    parser.add_argument(
    ...
    for entry in addresses:
        ...
        hifld_expected = entry["tier1_health_checks"]["hifld_power_lines"]
        expected_result = hifld_expected["expected_result"]

        # Run the actual check
        tier1 = check_hifld_power_lines(lat, lng, spatial_store)
        ...
        # Extract distance from the Tier1Check value (float meters, or None)
        actual_dist_m = tier1.value if isinstance(tier1.value, (int, float)) else None
        ...
        if args.verbose:
            ...
            dist_str = f"{actual_dist_m:.0f}m" if actual_dist_m is not None else "N/A"
            print(
    ...
    if mismatch_entries:
        ...
        for m in mismatch_entries[:20]:  # Cap at 20
            ...
            dist_str = (
                f"{m['actual_distance_m']:.0f}m" if m["actual_distance_m"] is not None
                else "N/A"
            )
            print(
                f"  {m['id']}: expected={m['expected_result']} "
                f"actual={m['actual_result']} "
                f"actual_dist={dist_str} "
                f"(generated {src.get('distance_m', '?')}m from "
                f"'{src.get('name', '?')}')"
            )
