...
def _ensure_table(conn):
    ...
    This allows the NJ script to run independently of the NYSED script.
...
def ingest(csv_path: str = "", **kwargs):
    ...
    Args:
        csv_path: Override path to CSV file (default: data/nj_district_performance.csv).
    """
    csv_file = csv_path or NJ_CSV_PATH
    if not os.path.exists(csv_file):
        logger.error("NJ performance CSV not found at %s", csv_file)
        return
    ...
    try:
        ...
        # Delete existing NJ rows (idempotent — preserves NY/CT data)
        deleted = conn.execute(
            f"DELETE FROM {TABLE_NAME} WHERE state = 'NJ'"
        ).rowcount
        if deleted > 0:
            logger.info("Cleared %d existing NJ rows", deleted)
        conn.commit()
        ...
        conn.execute(
            """INSERT OR REPLACE INTO dataset_registry
               ...
               VALUES (?, ?, ?, ?, ?)""",
            (
                "state_education_performance_nj",
                "nj.gov/education/spr/download (curated CSV)",
        ...
        conn.commit()
        logger.info("NJ performance data loaded: %d districts, %d skipped", total, skipped)
...
def verify():
    """Quick verification: look up Newark performance data."""
    ...
    try:
        cursor = conn.execute(
            f"SELECT * FROM {TABLE_NAME} WHERE state = 'NJ' ORDER BY district_name LIMIT 5"
        )
        ...
        if rows:
            ...
            logger.warning("No NJ data found in %s", TABLE_NAME)

        # Count
        count = conn.execute(
            f"SELECT COUNT(*) FROM {TABLE_NAME} WHERE state = 'NJ'"
        ).fetchone()[0]
        logger.info("Total NJ districts: %d", count)
    finally:
        conn.close()
