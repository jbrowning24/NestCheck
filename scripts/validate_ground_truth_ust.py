...
Loads ground-truth JSON from the generator, runs check_ust_proximity()
...
from property_evaluator import check_ust_proximity, CheckResult
from spatial_data import SpatialDataStore


def main():
    parser = argparse.ArgumentParser(
        description="Validate UST ground-truth against evaluator"
    )
    parser.add_argument(
        "--input", type=str, default="data/ground_truth_ust.json",
        help="Ground-truth JSON file (default: data/ground_truth_ust.json)",
    )
    parser.add_argument(
    ...
    for entry in addresses:
        ...
        ust_expected = entry["tier1_health_checks"]["ust_proximity"]
        expected_result = ust_expected["expected_result"]
        expected_pass = ust_expected["expected_pass"]

        # Run the actual check
        tier1 = check_ust_proximity(lat, lng, spatial_store)
