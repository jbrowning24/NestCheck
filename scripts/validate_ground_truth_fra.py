#!/usr/bin/env python3
"""
Validate FRA rail corridor ground-truth against the actual evaluator.

Loads ground-truth JSON from the generator, runs check_rail_proximity()
for each test coordinate, and compares actual vs. expected results.

No API calls — uses only spatial.db via SpatialDataStore.

Usage:
    python scripts/validate_ground_truth_fra.py
    python scripts/validate_ground_truth_fra.py --input data/ground_truth/fra.json
    python scripts/validate_ground_truth_fra.py --verbose
    python scripts/validate_ground_truth_fra.py --output data/validation_results_fra.json
"""
...
from property_evaluator import check_rail_proximity
from spatial_data import SpatialDataStore


def main():
    parser = argparse.ArgumentParser(
        description="Validate FRA ground-truth against evaluator"
    )
    parser.add_argument(
        "--input", type=str, default="data/ground_truth/fra.json",
        help="Ground-truth JSON file (default: data/ground_truth/fra.json)",
    )
    parser.add_argument(
    ...
    for entry in addresses:
        ...
        fra_expected = entry["tier1_health_checks"]["rail_proximity"]
        expected_result = fra_expected["expected_result"]

        # Run the actual check
        tier1 = check_rail_proximity(lat, lng, spatial_store)
        ...
        # Extract distance from the Tier1Check value
        actual_dist_m = None
        if isinstance(tier1.value, (int, float)):
            actual_dist_m = tier1.value
