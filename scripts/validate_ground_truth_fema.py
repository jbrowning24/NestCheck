#!/usr/bin/env python3
"""
Validate FEMA flood zone ground-truth against the actual evaluator.

Loads ground-truth JSON from the generator, runs check_flood_zones()
for each test coordinate, and compares actual vs. expected results.

No API calls — uses only spatial.db via SpatialDataStore.

Usage:
    python scripts/validate_ground_truth_fema.py
    python scripts/validate_ground_truth_fema.py --input data/ground_truth/fema.json
    python scripts/validate_ground_truth_fema.py --verbose
    python scripts/validate_ground_truth_fema.py --output data/validation_results_fema.json
"""
...
from property_evaluator import check_flood_zones


def main():
    parser = argparse.ArgumentParser(
        description="Validate FEMA flood zone ground-truth against evaluator"
    )
    parser.add_argument(
        "--input", type=str, default="data/ground_truth/fema.json",
        help="Ground-truth JSON file (default: data/ground_truth/fema.json)",
    )
    parser.add_argument(
    ...
    for entry in addresses:
        ...
        fema_expected = entry["tier1_health_checks"]["flood_zone"]
        expected_result = fema_expected["expected_result"]

        # Run the actual check
        # check_flood_zones() creates its own SpatialDataStore internally
        tier1 = check_flood_zones(lat, lng)
        ...
        if args.verbose:
            ...
            position = entry.get("source_polygon", {}).get("position", "?")
            print(
                f"[{marker}] {test_id}: expected={expected_result} "
                f"actual={actual_result} position={position}"
            )
            if status == "MISMATCH":
                src = entry.get("source_polygon", {})
                print(
                    f"       Zone={src.get('fld_zone', '?')} "
                    f"Position={position} "
                    f"| actual: {tier1.details}"
                )
