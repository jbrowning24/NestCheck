#!/usr/bin/env python3
"""
Validate SEMS (Superfund NPL) ground-truth against the actual evaluator.

Loads ground-truth JSON from the generator, runs check_superfund_npl()
for each test coordinate, and compares actual vs. expected results.

No API calls — uses only spatial.db via SpatialDataStore.

Usage:
    python scripts/validate_ground_truth_sems.py
    python scripts/validate_ground_truth_sems.py --input data/ground_truth/sems.json
    python scripts/validate_ground_truth_sems.py --verbose
    python scripts/validate_ground_truth_sems.py --output data/validation_results_sems.json
"""
...
from property_evaluator import check_superfund_npl


def main():
    parser = argparse.ArgumentParser(
        description="Validate SEMS ground-truth against evaluator"
    )
    parser.add_argument(
        "--input", type=str, default="data/ground_truth/sems.json",
        help="Ground-truth JSON file (default: data/ground_truth/sems.json)",
    )
    parser.add_argument(
    ...
    # Per-category tracking by containment_type
    by_category = {}  # containment_type -> {match, mismatch, unknown, total}

    for entry in addresses:
        test_id = entry["id"]
        lat = entry["coordinates"]["lat"]
        lng = entry["coordinates"]["lng"]
        sems_expected = entry["tier1_health_checks"]["superfund_npl"]
        expected_result = sems_expected["expected_result"]

        # Run the actual check
        tier1 = check_superfund_npl(lat, lng)
        actual_result = tier1.result.value  # "PASS", "FAIL", "UNKNOWN"
        ...
        # Track by containment_type category
        cat = entry.get("source_facility", {}).get("containment_type", "unknown")
        ...
        if args.verbose:
            ...
            site_str = tier1.value if tier1.value is not None else "N/A"
            print(
                f"[{marker}] {test_id}: expected={expected_result} "
                f"actual={actual_result} site={site_str}"
            )
            if status == "MISMATCH":
                src = entry.get("source_facility", {})
                print(
                    f"       Containment: {src.get('containment_type', '?')} | "
                    f"Facility: '{src.get('name', '?')}' "
                    f"(NPL status: {src.get('npl_status_code', '?')})"
                )
    ...
    print("Per-category breakdown (by containment type):")
    _known_cats = ["inside_npl", "outside", "inside_non_npl"]
    _all_cats = _known_cats + [c for c in by_category if c not in _known_cats]
    for cat in _all_cats:
        if cat in by_category:
            ...
            print(
                f"  {cat:16s}: {c['match']}/{cat_scored} correct "
    ...
    if mismatch_entries:
        ...
        for m in mismatch_entries[:20]:  # Cap at 20
            ...
            print(
                ...
                f"({src.get('lat', m['coordinates']['lat'])}, "
                f"{src.get('lng', m['coordinates']['lng'])}) "
                f"containment={src.get('containment_type', '?')} "
                f"facility='{src.get('name', '?')}'"
            )
