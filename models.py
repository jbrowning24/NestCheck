...
def create_queued_snapshot(address_input, trace_id=None, modules_status=None):
    ...
    Returns snapshot_id. The background worker will fill in results.
    """
    snapshot_id = generate_snapshot_id()
    now = datetime.now(timezone.utc).isoformat()
    conn = _get_db()
    ...
    if modules_status is not None:
        conn.execute(
            """UPDATE snapshots
               ...
               WHERE snapshot_id = ?""",
            (status, json.dumps(modules_status), now, snapshot_id),
        )
    else:
        conn.execute(
            """UPDATE snapshots
...
def update_snapshot_result(snapshot_id, result_dict, address_norm=None):
    ...
    Sets status='done' and populates verdict, scores, result_json.
    """
    now = datetime.now(timezone.utc).isoformat()
    conn = _get_db()
    conn.execute(
