...
- **PAC dropdown styling placement**: `.pac-container` and its children (`.pac-item`, `.pac-matched`, `.pac-icon`, etc.) are injected at `<body>` level by Google Maps JS — outside any template's DOM scope. Styles for these elements **must live in `base.css`** (globally loaded). Template-level `<style>` blocks won't apply. Use `!important` throughout: Google's injected stylesheet wins on specificity otherwise. When replacing icons via SVG data URIs, CSS variables don't resolve inside `url()` data URIs — hardcode the hex and add a comment mapping it to the design token (e.g., `/* #94A3B8 = --color-text-faint */`).
...
| 2026-03 | PAC autocomplete styles in base.css | `.pac-container` injected at `<body>` by Maps JS — template `<style>` blocks don't apply. All overrides need `!important`. SVG data URI colors must be hardcoded hex (CSS vars don't resolve in `url()`); comment which token the hex maps to |
