...
- **Cookie-based client state pattern**: `nestcheck_recent` cookie stores a JSON-encoded array of recent search addresses (max 5, 30-day expiry, `SameSite=Lax; Path=/`). Read with regex on `document.cookie`, write with `encodeURIComponent(JSON.stringify(...))`. Always wrap `JSON.parse` in try/catch and return `[]` on failure — cookies can be corrupted by browser extensions or manual edits. The `addRecentSearch` / `getRecentSearches` / `renderRecentSearches` functions in `index.html` are the reference implementation.
