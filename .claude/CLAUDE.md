...
- **HPMS display-name resolution** (`_build_road_display_name`): HPMS `ROUTE_ID` is a numeric segment identifier (e.g., "100503011"), not a road name. Use `route_name` (e.g., "SUNRISE HWY", "9A") → `segment.name` → `"Route {route_number}"` → `route_id` fallback chain. When building Tier1Check details for any result path (FAIL, WARNING, PASS-with-detail), always call `_build_road_display_name()` — don't inline the name resolution. Multiple code paths constructing the same detail string will diverge when the resolution logic changes.
