...
| 2026-03 | Shared visual component system (Phase 2) | `.summary-pill`, `.dim-score-pill`, `.mini-fill-bar`, `.section-card--*` modifiers in `report.css`. Pill variants: `--pass/--warning/--fail/--neutral/--accent/--compact`. Card modifiers: `--quiet` (opacity), `--highlighted` (accent border), `--elevated` (shadow). These are intentionally separate from `.health-stat-badge` (heavier, has border) and `.progress-track` (different sizing constraints) |
