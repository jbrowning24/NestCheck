...
- **New visual sub-palette vs. token reuse**: When a page intentionally introduces a neutral/monochrome sub-palette (e.g., `#111111`, `#8E8E8E`) that diverges from the site's blue-tinted token system, use tokens for *structural* values (spacing, font weights, border-radius, transitions) but leave palette colors as explicit values rather than mapping them to semantically mismatched tokens. Forcing `#111111` into `--color-text-primary: #0F172A` (navy) or `#8E8E8E` into `--color-text-muted: #64748b` (cool blue-gray) produces subtle hue drift that undermines the design intent.
...
| 2026-03 | Replaced persona pills with cookie-based recent searches | Persona selection removed from frontend entirely (backend defaults to None). Recent searches stored in `nestcheck_recent` cookie (JSON array, max 5, 30-day expiry). Section hidden when cookie is empty. Address dedup uses exact string match — acceptable since autocomplete normalizes most inputs |
